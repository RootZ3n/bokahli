...
After building, the same summaries can be printed from the read-only CLI:
...
The CLI lists executable benchmark metadata only. It does not load candidates, run verifiers, call models, or execute benchmark evaluation.

Candidate JSON files can be validated without running benchmark verification:
...
Validation exits `0` for valid candidates, `1` for candidate shape or benchmark mismatch failures, and `2` for usage or load failures such as missing files or malformed JSON.

Candidate JSON files can also be evaluated against deterministic executable fixtures:
