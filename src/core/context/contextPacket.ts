...
export async function buildContextPacket(input: ContextPacketInput): Promise<ContextPacket> {
  ...
  const packet: MutableContextPacket = {
    ...
    task: {
      ...
      forbiddenFiles: input.task.forbiddenFiles === undefined ? undefined : [...input.task.forbiddenFiles],
      verificationRequired: input.task.verificationRequired === undefined ? undefined : [...input.task.verificationRequired]
    },
    repoSummary: {
    ...
    truncation: {
      ...
      maxPacketChars: input.budgets?.maxPacketChars,
