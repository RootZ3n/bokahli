...
export interface ContextPacketExampleManifestTemplate {
  readonly id: string;
...
export interface ContextPacketExampleManifest {
  readonly version: 1;
  readonly generatedBy: string;
  readonly templates: readonly ContextPacketExampleManifestTemplate[];
}

export interface ContextPacketExampleManifestValidationError {
  readonly path: string;
  readonly code: string;
  readonly message: string;
}

export type ContextPacketExampleManifestValidationResult =
  | {
      readonly ok: true;
      readonly manifest: ContextPacketExampleManifest;
    }
  | {
      readonly ok: false;
      readonly errors: readonly ContextPacketExampleManifestValidationError[];
    };
...
function pushError(
  errors: ContextPacketExampleManifestValidationError[],
  errorPath: string,
  code: string,
  message: string
): void {
...
export function validateContextPacketExampleManifest(manifest: unknown): ContextPacketExampleManifestValidationResult {
  const errors: ContextPacketExampleManifestValidationError[] = [];
  ...
  if (!Array.isArray(templates)) {
    ...
    templates.forEach((template, index) => {
      ...
      if (typeof template["goal"] !== "string" || template["goal"].length === 0) {
        pushError(errors, `${templatePath}.goal`, "required_goal", "goal must be a non-empty string");
      }

      validateStringArray(errors, template["selectedPaths"], `${templatePath}.selectedPaths`, "selectedPaths");
  ...
  return {
    ok: true,
    manifest: manifest as unknown as ContextPacketExampleManifest
  };
}

export async function loadContextPacketExampleManifest(root = process.cwd()): Promise<ContextPacketExampleManifest> {
  const manifestPath = path.join(root, "examples/context-packets/manifest.json");
  const rawManifest = JSON.parse(await readFile(manifestPath, "utf8")) as unknown;
  const validation = validateContextPacketExampleManifest(rawManifest);

  if (!validation.ok) {
    const messages = validation.errors.map((error) => `${error.code} ${error.path}: ${error.message}`).join("; ");
    throw new Error(`invalid context packet example manifest: ${messages}`);
  }
